	...
	void Update () {
        ...
        if(Physics.Raycast(ray,out hitInfo))
        {
          //  Debug.Log(hitInfo.collider);
            if (hitInfo.collider.tag == Tags.Building)
            {
			...
			if (hitInfo.collider.tag == Tags.Shop)
			{
				if (Input.GetMouseButtonDown(0))
				{
					Camera.main.GetComponent<GUI_Temp>().leftClickedObj=hitInfo.collider.gameObject;
				}
			}
			if (hitInfo.collider.tag==Tags.Monster)
			{
				if (hitInfo.collider.GetComponent<Team>() != null && choosedObj.GetComponent<Team>() != null && hitInfo.collider.GetComponent<Team>().team != choosedObj.GetComponent<Team>().team)
				{
					Cursor.SetCursor(cursorAttack, hotSpot, cursorMode);
					...
					{
						//Debug.Log("feed");
						player.GetComponent<FeedMonster>().FeedSimple(hitInfo.collider.gameObject);
            ...
            else
            {
                ...
                if (Input.GetMouseButtonDown(1))
                {
					if(choosedObj.tag==Tags.Pet)return;
