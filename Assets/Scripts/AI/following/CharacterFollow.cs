...
public class CharacterFollow : MonoBehaviour
{
	...
	public void getFollow(float factor)

	{	
		float percentage = Random.Range (1.0F, 10.0F);
		percentage = percentage / 10;
		//Debug.LogError ("getFollow percentage =" + percentage); 

		float dis_percentage = Random.Range (-1.0F, 10.0F);
		dis_percentage = dis_percentage / 10;
		//Debug.LogError ("getFollow dis_percentage =" + dis_percentage); 
		...
		if (distance <= range2 && distance >= range1) {
			myTransform.rotation = Quaternion.Slerp (myTransform.rotation,
			                                         Quaternion.LookRotation (lookDir), percentage*rotationSpeed * Time.deltaTime);
		}
		else if (distance <=range1 && distance>stop){
			myTransform.rotation = Quaternion.Slerp (myTransform.rotation,
			                                         Quaternion.LookRotation (lookDir), percentage*rotationSpeed * Time.deltaTime);
			myTransform.position += myTransform.forward * moveSpeed * Time.deltaTime *dis_percentage;
		}
		else if (distance <= stop) {
			myTransform.rotation = Quaternion.Slerp (myTransform.rotation,
			                                         Quaternion.LookRotation (lookDir), percentage*rotationSpeed * Time.deltaTime);
